...
type JamsyncIgnorer struct {
	globs []glob.Glob
}

func NewIgnorer() *JamsyncIgnorer {
	return &JamsyncIgnorer{
		globs: make([]glob.Glob, 0),
	}
}

func (j *JamsyncIgnorer) ImportPatterns(filepath string) error {
...
func (j *JamsyncIgnorer) Match(filepath string) bool {
