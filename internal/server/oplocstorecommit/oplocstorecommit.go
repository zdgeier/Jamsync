...
func (s *LocalOpLocStore) filePath(ownerId string, projectId uint64, commitId uint64, pathHash []byte) string {
	return fmt.Sprintf("jb/%s/%d/oplocstorecommit/%d/%02X/%02X.locs", ownerId, projectId, commitId, pathHash[:1], pathHash)
}

func (s *LocalOpLocStore) fileDir(ownerId string, projectId uint64, commitId uint64, pathHash []byte) string {
	return fmt.Sprintf("jb/%s/%d/oplocstorecommit/%d/%02X", ownerId, projectId, commitId, pathHash[:1])
}
...
func (s *LocalOpLocStore) MaxCommitId(ownerId string, projectId uint64) (uint64, error) {
	...
	files, err := ioutil.ReadDir(fmt.Sprintf("jb/%s/%d/oplocstorecommit/", ownerId, projectId))
	if err != nil {
		log.Panic(err)
	}
...
func (s *LocalOpLocStore) DeleteProject(ownerId string, projectId uint64) error {
	return os.RemoveAll(fmt.Sprintf("jb/oplocs/%s/%d", ownerId, projectId))
}
